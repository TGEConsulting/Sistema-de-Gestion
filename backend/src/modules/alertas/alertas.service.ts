  ...
  for (const auditoria of auditorias) {
    if (esAuditoriaProxima(auditoria.fechaInicio, auditoria.estado, hoy)) {
      await emitirAlerta({
        ...
        mensaje: `La auditoría "${auditoria.alcance}" inicia el ${auditoria.fechaInicio.toLocaleDateString()}.`,
  ...
  for (const accion of acciones) {
    if (esAccionVencida(accion.fechaCompromiso, accion.estado, hoy)) {
      ...
      await emitirAlerta({
        ...
        mensaje: `La acción "${accion.descripcion}" venció el ${accion.fechaCompromiso.toLocaleDateString()}.`,
